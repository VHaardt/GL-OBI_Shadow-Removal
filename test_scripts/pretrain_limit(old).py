...
'''def erode_mask(mask, kernel_size=7):
    ...
    eroded_mask = F.conv2d(mask.float(), kernel, padding=kernel_size//2)
...
'''def blur_image_border(image, border_mask, blur_kernel_size):
    blurred_combined = image.clone()
    device = blurred_combined.device
    # Iterate over each image in the batch
    for i in range(image.size(0)):
        border_mask_i = border_mask[i, :, :, :].to(device)
        image = blurred_combined[i, :, :, :].clone()
        blurrer = v2.GaussianBlur(kernel_size=(blur_kernel_size, blur_kernel_size))

        blurred_img = blurrer(image)

        blurred_combined[i, :, :, :] = image * (1 - border_mask_i) + blurred_img * border_mask_i
...
if __name__ == "__main__":
    ...
    #milestones_p = [opt.decay_epoch + i * opt.decay_steps for i in range((opt.n_epochs - opt.decay_epoch) // opt.decay_steps)]#[me for me in range(opt.decay_epoch, opt.n_epochs, decay_step_p)]
    ...
    for epoch in range(1, opt.n_epochs + 1):
        ...
        for i, data in enumerate(train_loader):
            ...
            loss = opt.pixel_weight *  criterion_pixelwise(transformed_images[mask_exp != 0], gt[mask_exp != 0]) + opt.perceptual_weight * criterion_perceptual(blurred_innested_img, gt) #cambiato con blurred/innested
        ...
        if (epoch-1) % opt.valid_checkpoint == 0:
            with torch.no_grad():
                ...
                for idx, data in enumerate(val_loader):
                    ...
                    loss = opt.pixel_weight *  criterion_pixelwise(transformed_images[mask_exp != 0], gt[mask_exp != 0]) + opt.perceptual_weight * criterion_perceptual(blurred_innested_img, gt)
