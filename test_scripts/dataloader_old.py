...
class ISTDDataset(data.Dataset):
    ...
    def contour(self, mask):
        ...
        # Imposta una dimensione del kernel fissa di 15 pixel per l'espansione
        kernel_size = 40
        kernel = np.ones((kernel_size, kernel_size), np.uint8)
        padding = kernel_size // 2

        # Converti la maschera in uint8 se necessario
        if mask.dtype != np.uint8:
            mask = (mask * 255).astype(np.uint8)

        # Applica il padding alla maschera
        padded_mask = np.pad(mask, ((padding, padding), (padding, padding)), mode='constant', constant_values=0)

        # Dilatazione della maschera per espandere i bordi di 15 pixel
        dilated_mask = cv2.dilate(padded_mask, kernel, iterations=1)

        # Calcola il bordo come differenza tra la maschera dilatata e quella originale
        border_mask2 = dilated_mask - padded_mask
        penumbra = border_mask2

        # Rimuovi il padding dalla maschera penumbra
