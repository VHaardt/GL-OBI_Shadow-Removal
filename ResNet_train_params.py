...
if __name__ == "__main__":
    ...
    for epoch in range(1, opt.n_epochs + 1):
        ...
        for i, data in enumerate(train_loader):
            ...
            ###
            '''param = torch.empty(gt.shape[0], 6, device=device)
            for c in range(shadow.shape[1]):
                gt_ch = gt[:, c, :, :]
                mu_gt = torch.empty(gt.shape[0], device=device) # Forma: [batch_size]
                sd_gt = torch.empty(gt.shape[0], device=device)
                for b in range(gt.shape[0]):  
                    gt_b = gt[b, c, :, :] 
                    mask_b = eroded_mask[b, 0, :, :]  
                    med = torch.mean(gt_b[mask_b != 0])
                    stand = torch.std(gt_b[mask_b != 0])
                    mu_gt[b] = med.item()
                    sd_gt[b] = max(stand.item(), 1e-5)
                param[:, 2 * c] = sd_gt
                param[:, 2 * c + 1] = mu_gt'''
            ###

            inp_p = torch.cat((shadow, norm_img,eroded_mask), dim=1)
            # Pass through ResNet model
            optimizer_p.zero_grad()
            out_p = resnet(inp_p)

            loss = criterion_mse(out_p, param)
            ...
            with torch.no_grad():
                ...
                if opt.save_images and (epoch == 1 or epoch % 50 == 0):
                    ...
                    for count in range(len(shadow)):
                        ...
                        exp_img = torch.clamp(exp_img, 0, 1)
                        innested_img = torch.clamp(innested_img, 0, 1)
                        gt = torch.clamp(gt, 0, 1)

                        im_input = (shadow[count].detach().cpu().numpy().transpose(1, 2, 0) * 255).astype(np.uint8)
                        norm_img = (norm_img[count].detach().cpu().numpy().transpose(1, 2, 0) * 255).astype(np.uint8)
                        exp_img = (exp_img[count].detach().cpu().numpy().transpose(1, 2, 0) * 255).astype(np.uint8)
                        ...
                        im_conc = np.concatenate((im_input, norm_img, exp_img, im_innest, im_gt), axis=1)
