...
if __name__ == "__main__":
    ...
    train_dataset = Dataset(os.path.join(opt.dataset_path, "train"), size=(opt.img_height, opt.img_width), aug=True, fix_color=True, masks_precomp=True)
    val_dataset = Dataset(os.path.join(opt.dataset_path, "test"), aug=False, fix_color=True, masks_precomp=True)
        ...
        if (epoch) % opt.valid_checkpoint == 0 or epoch == 1:
            with torch.no_grad():
                ...
                for idx, data in enumerate(val_loader):
                    inp = data['shadow_image'].type(Tensor)
                    gt = data['shadow_free_image'].type(Tensor)
                    mask = data['shadow_mask'].type(Tensor)
